...
import VirtualDOMHandleReverse from "@/common/VirtualDOMHandleReverse";
...
const containerId = 'diagramContainerReverse';
...
const mapStateToProps = (state) => {
  return {
    ...
    reRender: state.getIn(['globalReverse','reRender']),
    mainContainerHeight: state.getIn(['globalReverse', 'mainContainerHeight']),
    freeConnectStyle: state.getIn(['globalReverse', 'commonFreeConnectStyle']),
    isClearCont: state.getIn(['globalReverse', 'isClearCont']),
    dataJson: state.getIn(['globalReverse', 'dataJson']),
    mainContainerPosition: state.getIn(['globalReverse', 'mainContainerPosition'])
  }
}
...
const mapDispatchToProps = (dispatch) => {
  return {
    handleNodelistData(nodeList){
      const action = actionCreatorsGlobalReverse.setNodelistData(nodeList);
      dispatch(action);
    },
    ...
    setMainContainerHeight(mainHeight){
      const action = actionCreatorsGlobalReverse.setMainContainerHeight(mainHeight);
      dispatch(action);
    },
    handleSetProperty(info){
      const action = actionCreatorsPropertyReverse.setProperty(info)
      dispatch(action);
    },
    ...
    handleClearOut(booleanData){
      const action = actionCreatorsGlobalReverse.clearOut(booleanData);
      dispatch(action);
    },
    saveWorkFlow(dataItem, requestUrl){
      dispatch(actionCreatorsGlobalReverse.saveWorkFlow(dataItem, requestUrl));
    },
    setNodeFlowMenu(dataItem){
      dispatch(actionCreatorsGlobalReverse.setNodeFlowMenu(dataItem));
    },
    setWorkFlowMode(dataItem) {
      dispatch(actionCreatorsGlobalReverse.setWorkFlowMode(dataItem));
    }
  }
};
...
export default class Index extends Component {
  ...
  constructor(props) {
    ...
    this.jsPlumbReverse = jsPlumb.getInstance();
    this.VirtualDomClassReverse = null;
    ...
    this.workFlowJump = this.workFlowJump.bind(this);
    ...
    this.setConfig = this.setConfig.bind(this); //设置初始化一些配置信息
  ...
  setConfig(requestConfig) {
    ...
    //console.log("反Component", Component.prototype);
  }
  ...
  getData(id){
    ...
    axios.get(`${ this.jsPlumbConfig.reverseWorkFlowUrl }?bpmnId=${id}`)
        .then(res =>{
          const { data: { data } } = res;
          ...
          /**
           * 特别注释:这里反渲染开始的时候，拿到数据源的时候需要把节点以及连线的信息
           * 里面的id增加前缀或者是后缀，要不然当用户同时打开工作流定义、工作流列表的
           * 时候，页面同时有多个相同id的节点，就会产生渲染的错误bug，单页面应用就是这样
           */
          dataSource.nodeData.forEach( (item) => {
            if( item.id.indexOf("Reverse") === -1) {
              item.id = `${item.id}Reverse`
            }
          });
          dataSource.connectionData.forEach( (item) => {
            if( item.source.elementId.indexOf("Reverse") === -1) {
              item.source.elementId = `${item.source.elementId}Reverse`;
            }
            if( item.target.elementId.indexOf("Reverse") === -1) {
              item.target.elementId = `${item.target.elementId}Reverse`;
            }
          });
          setDataSource(fromJS(dataSource));
          this.initFlow(dataSource);
        })
        .catch((error)=>{
  ...
  initFlow() {
    ...
    this.jsPlumbReverse.ready(() => {
      // 设置绘图容器
      this.jsPlumbReverse.setContainer(containerId)
      // 可以使用importDefaults，来重写某些默认设置
      this.jsPlumbReverse.importDefaults({
  ...
  // 设置指定节点可拖动
  setDraggable(id) {
    this.jsPlumbReverse.draggable(id, {
  ...
  addEndpoint(id, xname, yname) {
    ...
    this.jsPlumbReverse.addEndpoint(id, {anchors: anchors, uuid: `${id}-anchor-${xname}-${yname}`}, commonConfig)
  }
  ...
  clearCont() {
     ...
     */
    this.jsPlumbReverse.deleteEveryEndpoint();
    /**
     * 清空数据源数据、属性值、清空虚拟dom数据、清空虚拟dom类数据源
     */
    setDataSource(fromJS(proConfig.dataSource));
    handleNodelistData(fromJS([]));
    handleSetProperty(fromJS({}))
    this.VirtualDomClassReverse.clearData( proConfig.dataSource );
  }

  clearContUnmount() {
    const { data4, setDataSource, handleNodelistData, handleSetProperty } = this.props;
    this.jsPlumbReverse.reset();
    /**
  ...
  getConnectionData() {
     ...
     * @type {Object}
     */
    const originalData = this.jsPlumbReverse.getAllConnections()
  ...
  loadDataAndPaint() {
     ...
     */
    this.VirtualDomClassReverse = new VirtualDOMHandleReverse(data4, eventObj, extraEventObj);
    this.VirtualDomClassReverse.init({ cursor: "move"});

    /**
     * 这个阶段渲染的只是流程的节点数据，并没有处理每个节点的拖动情况，连线情况等，
     * 安排在setState回调函数中去处理节点的连线、拖动等属性
     */
    this.props.handleNodelistData(fromJS(this.VirtualDomClassReverse.nodeList));
    /**
  ...
  upDateAndPaint(propsDataSource, extraPropsObj, draggable){
    ...
    this.VirtualDomClassReverse.updateNodeFlow(nodeData, eventObj, extraPropsObj);
    console.log("*******************upDate nodeList*****************************", this.VirtualDomClassReverse.nodeList);
    /**
     * 这个阶段渲染的只是流程的节点数据，并没有处理每个节点的拖动情况，连线情况等，
     * 安排在setState回调函数中去处理节点的连线、拖动等属性
     */
  ...
  handleDomMouseUp(event, info) {
    ...
    if (event.button === 2) {
      console.log("点击了鼠标右键", event.clientX);
  ...
  bindDeleteConnection() {
    this.jsPlumbReverse.bind('dblclick', (connection) => {
   ...
   */
  bindGetFlowInfo() {
    this.jsPlumbReverse.bind('click', (connection) => {
  ...
  bindConnectionAddLabel() {
    ...
    this.jsPlumbReverse.bind('beforeDrop',  (info) => {
   ...
   */
  saveDataHandle(){
    const { data4 :{ nodeData, connectionData, bpmnAttr } , setDataSource, saveWorkFlow, id } = this.props;
     ...
     * @type {Object | Array}
     */
    const newOriginalData = this.jsPlumbReverse.getAllConnections()
  ...
  /**
   * 这里是当一个节点的type为input时，dbclick以后这边的光标会默认移至末尾
   * -----这里的操作和jsPlumb无关，标记一下-----
   * @param ele
   */
  moveEnd(ele) {
    ele.focus();
  ...
  nodeFlowCanDraggable(nodeData){
    ...
    nodeData.forEach((info) => {
       ...
       * @type {boolean}
       */
      let flag = this.jsPlumbReverse.toggleDraggable(info.id);
      if (!flag) {
        this.jsPlumbReverse.toggleDraggable(info.id);
      }
      /**
       * 切换成鼠标工具后，必须解除Source、Target的资源id对应的dom操作，
       * 否则拖动后还是会继续进行连线操作
       */
      this.jsPlumbReverse.unmakeSource(info.id);
      this.jsPlumbReverse.unmakeTarget(info.id);
    });
    /**
     * 改变工具按钮的focus状态
     */
  ...
  nodeFlowCanConnection(nodeData){
    ...
    nodeData.forEach((info) => {
       ...
       */
      this.jsPlumbReverse.makeSource(info.id, this.props.freeConnectStyle.toJS());
      this.jsPlumbReverse.makeTarget(info.id, this.props.freeConnectStyle.toJS());
    });
    /**
     * 改变工具按钮的focus状态
     */
  ...
  handleDropDown(dropInfo){
    ...
    /**
     * 在这里虚拟dom类执行的一定是新增的方法，因为在初始化的时候已经实例化
     */
    this.VirtualDomClassReverse.addNodeFlow( newNodeData, dropInfo, eventObj);
    console.log("reCreatorData", this.VirtualDomClassReverse);
    console.log("reCreatorData.nodeList", this.VirtualDomClassReverse.nodeList);
    this.props.handleNodelistData(fromJS(this.VirtualDomClassReverse.nodeList));
    /**
  ...
  handleConnectionOk() {
    ...
    this.setState(() => ({
      ...
      this.jsPlumbReverse.deleteConnection(this.state.currentConnection);
  ...
  render() {
    return (
        <div style={{ position: "relative" }}>
          ...
          <NodeFlowRightMenu
              mode= {"reverse"}
              jsplumbGetAllConnections={ this.jsPlumbReverse.getAllConnections }
              jsplumbDeleteConnectionsForElement={ (item) => { this.jsPlumbReverse.deleteEndpoint(item) } }
              jsplumbDeleteConnection={ (item) => { this.jsPlumbReverse.deleteConnection(item) } }
          />
        </div>
    )
  }
